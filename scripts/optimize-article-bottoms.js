 ...
 * 2. Replace [Oopbuy Spreadsheet](docs.google.com) -> [Oopbuy Spreadsheet](https://usfanslinki.com/)
...
function processArticle(filePath) {
  ...
  // Step 2: Check for existing Oopbuy Spreadsheet anchor (case-insensitive)
  const ksRegex = /\[(oopbuy spreadsheet)\]\(([^)]+)\)/gi;
  ...
  if (!hasKsAnchor) {
    ...
    const summaryNoLink = summary.replace(
      'visit the [Oopbuy Spreadsheet](https://usfanslinki.com/) homepage',
      'visit the Oopbuy Spreadsheet homepage'
    );
    content = content.trimEnd() + '\n\n' + summaryNoLink + '\n';
  }
